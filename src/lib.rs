...
mod flux_addition {
    enable_method_auth! {
        methods {
            ...
            set_min_debt_before_close => restrict_to: [OWNER];
    ...
    extern_blueprint! {
        ...
        Flux {
            fn free_fusd(&mut self, amount: Decimal) -> Bucket;
            fn close_cdp(&mut self, cdp_id: NonFungibleLocalId, fusd_payment: Bucket) -> (Bucket, Bucket);
            fn partial_close_cdp(&mut self, cdp_id: NonFungibleLocalId, repayment: Bucket) -> (Option<Bucket>, Option<Bucket>);
            fn remove_collateral(&mut self, cdp_id: NonFungibleLocalId, amount: Decimal, with_price: Option<Decimal>) -> Bucket;
        }
    }
    ...
    impl FluxAddition {
        pub fn instantiate(
            ...
            Self {
                usd_tokens: KeyValueStore::new_with_registered_type(),
                retrievable_collateral: KeyValueStore::new_with_registered_type(),
                cdp_address,
                fine: dec!("1.1"),
                min_debt_before_close: dec!("0.1"),
                fusd_address,
        ...
        pub fn mint_with_usd(&mut self, usd: FungibleBucket) -> Bucket {
            ...
            assert!(usd_token.accepted == true, "Token not accepted");
        ...
        pub fn set_min_debt_before_close(&mut self, min_debt_before_close: Decimal) {
            self.min_debt_before_close = min_debt_before_close;
        }
        ...
        pub fn close_loan(
            ...
            let (collateral, leftover_fusd): (Bucket, Bucket) = self
                .badge_vault
                .authorize_with_amount(badge_amount, || FLUX.close_cdp(cdp_id, fusd));
            let fusd_spent: Decimal = fusd_input_amount - leftover_fusd.amount();

            assert!(
                fusd_spent < self.min_debt_before_close,
        ...
        pub fn partial_liquidate_cdp(
            ...
            let cdp_manager: ResourceManager = ResourceManager::from_address(self.cdp_address);
            let receipt_data: Cdp = cdp_manager.get_non_fungible_data(&cdp_id);

            let stability_pool_infos = STABILITY_POOLS
                .get_stability_pool_infos(Some(vec![receipt_data.collateral_address.clone()]));
            ...
            let collateral_resource_manager =
                ResourceManager::from_address(receipt_data.collateral_address);
            let mut collateral_bucket = match collateral {
                Some(collateral) => collateral,
                None => collateral_resource_manager.create_empty_bucket(),
            };
            let collateral_amount: Decimal = collateral_bucket.amount();

            let fusd_resource_manager = ResourceManager::from_address(self.fusd_address);
            let leftover_fusd_bucket = match leftover_fusd {
                Some(leftover_fusd) => leftover_fusd,
                None => fusd_resource_manager.create_empty_bucket(),
            };
            ...
            let extra_collateral_to_take = max_collateral_to_take - collateral_amount;
            let collateral_to_take_out_to_reach_min =
                receipt_data_after_close.collateral_amount - (Decimal::ONE / collateral_price); //collateral available - min collateral in cdp

            let extra_collateral: Bucket = if extra_collateral_to_take > Decimal::ZERO {
                let amount_to_remove =
                    extra_collateral_to_take.min(collateral_to_take_out_to_reach_min);
                let high_price = collateral_price * dec!("9999");
                self.badge_vault.authorize_with_amount(badge_amount, || {
                    FLUX.remove_collateral(cdp_id.clone(), amount_to_remove, Some(high_price))
                })
            } else {
                collateral_resource_manager.create_empty_bucket()
            };
